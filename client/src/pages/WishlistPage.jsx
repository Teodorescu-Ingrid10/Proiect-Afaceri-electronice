...
export default function WishlistPage() {
    ...
    const loadWishlist = async () => {
      try {
        ...
        const productIds = items
          .map((it) => it.productId ?? it.product_id ?? it.product?.id)
          .filter(Boolean);
        ...
        const byId = Object.fromEntries(
          productsList.map((p) => [p.id ?? p._id, p])
        );
  ...
  const handleSaveNotes = async () => {
    try {
      const response = await updateWishlistItem(editingId, {
        notes: editNotes,
      });
      if (response?.success) {
        setWishlistItems(
